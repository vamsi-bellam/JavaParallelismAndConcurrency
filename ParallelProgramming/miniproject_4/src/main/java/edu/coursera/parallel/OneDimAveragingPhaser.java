        ...
        for (int ii = 0; ii < tasks; ii++) {
            ...
            threads[ii] = new Thread(() -> {
                ...
                for (int iter = 0; iter < iterations; iter++) {
                    ...
                    // this is required for previous task's last element in that chunk
                    threadPrivateMyNew[left] = (threadPrivateMyVal[left-1] + threadPrivateMyVal[left+1])/2.0;

                    // this is required for next task's first element in that chunk
                    threadPrivateMyNew[right] = (threadPrivateMyVal[right-1] + threadPrivateMyVal[right+1])/2.0;

                    // here we signal that other tasks can use this and continue work
                    ...
                    // here we say, wait for all above to complete for next iteration
